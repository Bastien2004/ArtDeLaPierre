...
<head>
    ...
    <style>
        ...
        .epaisseur-badge {
            ...
            padding: 8px 20px; font-size: 14pt; font-weight: bold; border-radius: 4px;
        }
        ...
        .grand-total-box {
            margin-top: 50px; background: #2c3e50; color: white; padding: 20px;
...
@foreach($stocksGroupes as $epaisseur => $items)
    @php
        ...
        $prixUnitaire = $tarifs[$epaisseur] ?? 0;
        $valeurEpaisseur = $surfaceEpaisseur * $prixUnitaire;
    @endphp
        ...
        <table>
            ...
            <tr>
                <td colspan="4" class="txt-right price-tag">
                    Estimation Valeur ({{ $prixUnitaire }}€/m²) : {{ number_format($valeurEpaisseur, 2, ',', ' ') }} €
                </td>
            </tr>
            </tbody>
        </table>
    </div>
    @php $totalSurfaceGlobale += $surfaceEpaisseur; @endphp
@endforeach

<div class="grand-total-box">
    <table style="color: white; margin: 0;">
        <tr>
            <td style="font-size: 16pt; border: none;">SURFACE TOTALE EN STOCK</td>
            <td class="txt-right" style="font-size: 20pt; border: none; font-weight: bold;">
                {{ number_format($totalSurfaceGlobale, 2, ',', ' ') }} m²
            </td>
        </tr>
    </table>
</div>
